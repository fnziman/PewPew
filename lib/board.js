...
class Board {
  ...
  checkGameOver() {
    if (!this.grid[0].every(el => el === 0)) {
      this.gameOver = true;
      DOM_ELS.gameOver.css('display', 'inline-block');
      DOM_ELS.finalScore.text(`Score: ${this.game.score}`);
    }
  }
  setHighScore() {
  ...
  dropPew() {
    ...
    if (!this.gameOver) {
      ...
      document.addEventListener("keydown", this.move); //how to change to jQuery?
      this.createPew();
