...
class Game {
  constructor(canvas, ctx) {
    ...
    getHighScore().then((res) => {
      this.highScore = res;
      DOM_ELS.highScore.text(`High Score: ${res}`);
    });
  ...
  pause() {
    ...
    DOM_ELS.paused.css('display') === 'none' ? DOM_ELS.paused.css('display', 'inline-block') : DOM_ELS.paused.css('display', 'none');
    //Better way to write this ^^^^ line?
    if (this.playing) {
  ...
  destroy(toClear) {
    ...
    DOM_ELS.score.text(`Score: ${this.score}`);
    if (this.score >= this.highScore) {
      setHighScore(this.score);
      this.highScore = this.score;
      DOM_ELS.highScore.text(`High Score: ${this.highScore}`);
    }
    toClear.forEach((pos) => {
